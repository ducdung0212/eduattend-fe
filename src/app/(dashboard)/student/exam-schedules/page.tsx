...
const COLUMNS: Column<ExamSchedule>[] = [
    ...
    },
    {
        ...
        className: "w-[90px]",
        render: (row) => {
            const endDate = new Date(new Date(row.start_time).getTime() + (row.duration ?? 120) * 60000);
            return (
                <span className="whitespace-nowrap">
                    <span className="text-xs">{row.duration} phút</span>
                    <span className="text-xs text-slate-400 ml-1">(→{formatTime(endDate)})</span>
                </span>
            );
        },
    },
